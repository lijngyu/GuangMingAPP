...
import com.example.litianci.guangming.bean.YongzhangResult;
...
import com.example.litianci.guangming.xiangqing.YongZhangXQActivity;
...
public class YongZhangActivity extends BaseActivity implements PullToRefreshSwipeMenuListView.IXListViewListener, AbsListView.OnScrollListener {
    ...
    @Bind(R.id.iv_search)
    ImageView ivSearch;
    private List<YongzhangResult.RstBean.ListBean> firstList;
    private List<YongzhangResult.RstBean.ListBean> allList = new ArrayList<>();
    private CommonAdapter<YongzhangResult.RstBean.ListBean> resultAdapter;
    private YongzhangResult result;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvTitle.setText("用章查询");
        searchEtTz.setHint("请输入申请人或申请部门");
        ...
        gwglList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent intent = new Intent(YongZhangActivity.this, YongZhangXQActivity.class);
    ...
    public void getRequest(String string) {
        ...
        params.put(Globals.WS_POST_KEY, "{\"Ac\": \"Seal\",\"Para\": {\"Sid\": \"" + User.sid + "\",\"Search\": \"" + string + "\",\"Page\": \"" + page + "\"}}");

        new VolleyUtil() {

            @Override
            public <T> boolean analysisData(String response) {
                result = GsonUtils.json2bean(response, YongzhangResult.class);
                Log.i("wwwwwww", response);
                if (result == null || !(result.getStu() == 1)) {
                    Toast.makeText(YongZhangActivity.this, Globals.SER_ERROR,
                    ...
                    if (flsh) {
                        ...
                        RefreshTime.setRefreshTime(YongZhangActivity.this, df.format(new Date()));
                        resultAdapter.setData(firstList);
                        resultAdapter.notifyDataSetChanged();
                        gwglList.setRefreshTime(RefreshTime.getRefreshTime(YongZhangActivity.this));
                        ...
                        if (isFlsh) {
                            ...
                            resultAdapter = new CommonAdapter<YongzhangResult.RstBean.ListBean>(YongZhangActivity.this, firstList, R.layout.item_list_yongzhang) {
                                @Override
                                public void convert(ViewHolder holder, YongzhangResult.RstBean.ListBean testResult) {
                                    holder.setText(R.id.tv_title, testResult.getSealName()).
                                            setText(R.id.tv_person, testResult.getPerson()).
                                            setText(R.id.tv_time, testResult.getTime()).
                                            setText(R.id.tv_dept, testResult.getDept());
                                    if (testResult.getSealType().contains("0")) {
                                        holder.setImageResource(R.id.image, R.mipmap.zhengfuzhang);
                                    } else {
                                        holder.setImageResource(R.id.image, R.mipmap.dangweizhang);
                                    }
                                }
                            };
                            ...
                            if (firstList.size() != 0) {
                                ...
                                Toast.makeText(YongZhangActivity.this, "没有更多数据了", Toast.LENGTH_SHORT).show();
                                onLoad();
                            }
                        }
        ...
        }.volleyStringRequestPost(YongZhangActivity.this, params, "app?", null);
    }
